...
/**
 * UnifiedSidebar
 * The main control panel for the application. Dark-only styling.
 */
export function UnifiedSidebar({
  ...
  const panelBase = "bg-zinc-900/80 border-zinc-800 text-zinc-100 shadow-zinc-950/40";
  const headerBorder = "border-zinc-800 bg-zinc-900/50";
  const inputBg = "bg-zinc-950 border-zinc-800 text-zinc-100 focus:ring-violet-400/20 shadow-none";
  const selectorBg = "bg-zinc-800/40 border-zinc-800";
  const selectorActive = "bg-zinc-700 text-violet-400";
  const logCardBg = "bg-zinc-800/30 border-zinc-800 hover:bg-zinc-800/60";
    ...
    <div className={`absolute top-4 bottom-4 left-4 right-4 min-[660px]:left-auto min-[660px]:top-10 min-[660px]:right-10 min-[660px]:bottom-10 min-[660px]:w-96 glass-panel rounded-[2.5rem] flex flex-col z-40 overflow-hidden shadow-2xl transition-all border border-white/20 ${panelBase}`}>
          ...
          <div className="w-full pr-4">
            ...
            <div className="relative">
              <select 
                ...
                className={`appearance-none w-full rounded-xl border px-3 py-2 pr-8 text-xs font-bold transition-colors focus:outline-none focus:ring-2 focus:ring-violet-500/20 cursor-pointer ${inputBg} border-zinc-800`}
              >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-6 pt-1.5 pb-6 space-y-2">
        ...
        <section className="space-y-1.5">
                ...
                return (
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl flex flex-col items-center gap-1 transition-all ${
                      isActive
                        ? (isItemDisabled ? 'bg-zinc-800 text-zinc-500' : selectorActive)
                        : (isItemDisabled ? 'text-zinc-500 opacity-50' : 'text-zinc-500 hover:text-zinc-400')
          ...
          {isInvalidConfig && (
             <div className="p-3 rounded-xl border text-[10px] leading-relaxed animate-in slide-in-from-top-1 fade-in duration-200 bg-amber-500/10 border-amber-500/20 text-amber-200">
                <p>
        ...
        <section className="space-y-2">
          <div className="relative group">
            ...
            <button
              ...
              className={`absolute top-1/2 right-2 -translate-y-1/2 p-1.5 rounded-xl transition-colors ${showSettings ? 'text-violet-400 bg-zinc-800' : 'text-zinc-500 hover:text-zinc-300'}`}
          ...
          {showSettings && (
            <div className="flex items-center justify-between gap-4 px-1 py-1 animate-in slide-in-from-top-2 fade-in duration-200 bg-zinc-800/20 rounded-xl">
               <div className="flex-1 flex flex-col gap-1.5 p-2">
                  <div className="flex justify-between items-end">
                     ...
                     <span className="text-[10px] font-mono font-bold text-zinc-300">{temperature}</span>
                  </div>
                  <input 
                    type="range" 
                    min="0" 
               ...
               <div className="flex items-center gap-2 pt-4 p-2" title="Thinking improves the capabilities of the model to reason through tasks, but may produce less desirable results for simple locating tasks. For simple tasks, disable thinking for improved speed and likely better results.">
                   ...
                   />
                   <label htmlFor="thinking-toggle" className="text-[9px] font-bold uppercase tracking-widest cursor-pointer select-none text-zinc-300">
                      Thinking
                   </label>
               </div>
            </div>
            ...
            <button 
                ...
                className={`flex-1 py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                  isLoading || isInvalidConfig 
                    ...
                    : 'bg-zinc-100 text-zinc-900 hover:bg-white shadow-lg active:scale-[0.98]'
                }`}
            >
        ...
        <section className="space-y-2 pt-2">
          ...
          <div className="space-y-3">
            {logs.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed rounded-[2rem] text-xs italic border-zinc-800 text-zinc-600">
                No history found
              </div>
            ) : (
              logs.map((log) => {
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 py-1">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-[8px] font-bold px-1.5 py-0.5 rounded uppercase tracking-tighter bg-zinc-800 text-zinc-400">
                          {typeLabel}
                        </span>
                      </div>
                      <p className="text-[11px] font-semibold truncate text-zinc-200">{log.prompt}</p>
                      {log.result === null ? (
                        ...
                        <div className="flex items-center justify-between mt-2 gap-2">
                           ...
                           <button 
                             ...
                             className="shrink-0 flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-bold transition-all bg-red-500/10 text-red-400 hover:bg-red-500/20"
                           >
